...
This enforces:
- ❌ No inline `<script>alert()</script>` — use external `.js` files
- ❌ No `eval()`, `new Function()`, `setTimeout("string")`
- ❌ No external CDN scripts (`<script src="https://...">`)
- ✅ Scripts from extension package only (`src="popup.js"`)
...
**You cannot add `'unsafe-inline'` or `'unsafe-eval'` to `extension_pages`** — Chrome Web Store will reject it.

---

## Sandboxed Pages (for eval use cases)

If you need `eval()` (e.g., for a templating engine), use a sandboxed page:
...
```js
// From popup or service worker — send to sandbox
