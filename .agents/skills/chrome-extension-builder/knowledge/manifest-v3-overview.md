# Manifest V3 — Complete Overview & MV2 Migration Guide

## What is Manifest V3?

Manifest V3 (MV3) is the current required format for Chrome extensions. As of June 2024, Chrome Web Store no longer accepts new MV2 extensions. Existing MV2 extensions stopped working in Chrome stable in January 2025.

MV3 key changes from MV2:
- Background pages replaced by **service workers** (ephemeral, not persistent)
- `chrome.tabs.executeScript` replaced by `chrome.scripting.executeScript`
- `webRequest` blocking replaced by `declarativeNetRequest`
- Stricter Content Security Policy (no remotely hosted code, no eval)
...
```json
{
  ...
  "background": {
    ...
    "type": "module"                // Optional: enables ES modules in SW
  },
  ...
  "name": "__MSG_appName__",        // Use message key for localization
  ...
  // ─── Native Messaging ─────────────────────────────────────────────────────
  "natively_connected_applications": ["com.example.app"],
...
| `eval()` / `new Function()` | ❌ Not allowed in extension pages |
